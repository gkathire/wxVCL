...
wxDateTime RecodeMinute(const wxDateTime & AValue,
  const unsigned short & AMinute)
{
	return wxDateTime(AValue).SetMinute(AMinute);
}

wxDateTime RecodeSecond(const wxDateTime & AValue,
  const unsigned short & ASecond)
{
	return wxDateTime(AValue).SetSecond(ASecond);
}

wxDateTime RecodeMilliSecond(const wxDateTime & AValue,
  const unsigned short & AMilliSecond)
{
	return wxDateTime(AValue).SetMillisecond(AMilliSecond);
}

wxDateTime RecodeDate(const wxDateTime & AValue, const unsigned short & AYear,
  const unsigned short & AMonth, const unsigned short & ADay)
{
	return wxDateTime(AValue).Set(AYear, AMonth, ADay);
}

wxDateTime RecodeTime(const wxDateTime & AValue, const unsigned short & AHour,
  const unsigned short & AMinute, const unsigned short & ASecond,
  const unsigned short & AMilliSecond)
{
	return wxDateTime(AValue).Set(AHour, AMinute, ASecond, AMilliSecond);
}

wxDateTime RecodeDateTime(const wxDateTime & AValue,
  const unsigned short & AYear, const unsigned short & AMonth,
  const unsigned short & ADay, const unsigned short & AHour,
  const unsigned short & AMinute, const unsigned short & ASecond,
  const unsigned short & AMilliSecond)
{
	return wxDateTime(AValue).Set(ADay, (wxDateTime::Month) AMonth, AYear,
	  AHour, AMinute, ASecond, AMilliSecond);
}

bool TryRecodeDateTime(const wxDateTime & AValue, const unsigned short & AYear,
  const unsigned short & AMonth, const unsigned short & ADay,
  const unsigned short & AHour, const unsigned short & AMinute,
  const unsigned short & ASecond, const unsigned short & AMilliSecond,
  wxDateTime & AResult)
{
	...
	return TryEncodeDateTime(AYear, AMonth, ADay, AHour, AMinute, ASecond,
	  AMilliSecond, AResult);
}
...
void DecodeDayOfWeekInMonth(const wxDateTime & AValue, unsigned short & AYear,
  unsigned short & AMonth, unsigned short & ANthDayOfWeek,
  unsigned short & ADayOfWeek)
{
	unsigned short D = 0;
...
wxDateTime EncodeDayOfWeekInMonth(const unsigned short & AYear,
  const unsigned short & AMonth, const unsigned short & ANthDayOfWeek,
  const unsigned short & ADayOfWeek)
{
	wxDateTime Result;
	if (!TryEncodeDayOfWeekInMonth(AYear, AMonth, ANthDayOfWeek, ADayOfWeek,
	  Result))
		InvalidDayOfWeekInMonthError(AYear, AMonth, ANthDayOfWeek, ADayOfWeek);
	return Result;
}

bool TryEncodeDayOfWeekInMonth(const unsigned short & AYear,
  const unsigned short & AMonth, const unsigned short & ANthDayOfWeek,
  const unsigned short & ADayOfWeek, wxDateTime & AValue)
{
...
wxString DoField(unsigned int Arg, unsigned short Def, wxString Unknown,
  const wxDateTime &ABaseDate)
{
	wxUnusedVar(Def);
	wxString Result;
	if ((Arg != LFAI))
		Result = Format(wxT("%.*d"), Length(Unknown), Arg);
	else if (ABaseDate.IsValid() == false)
		Result = Unknown;
	else
		Result = Format(wxT("%.*d"), Length(Unknown), Arg);
	return Result;
...
void InvalidDateTimeError(const wxDateTime & ErrorDate,
  const wxDateTime & ABaseDate)
{
	unsigned short Y, M, D, H, N, S, MS;
	wxString Msg;
	...
	Msg = Msg + DateSeparator + DoField(ErrorDate.GetMonth(), M, wxT("??"),
	  ABaseDate);
	Msg = Msg + DateSeparator + DoField(ErrorDate.GetDay(), D, wxT("??"),
	  ABaseDate);
	Msg = Msg + wxT(" ") + DoField(ErrorDate.GetHour(), H, wxT("??"),
	  ABaseDate);
	Msg = Msg + TimeSeparator + DoField(ErrorDate.GetMinute(), N, wxT("??"),
	  ABaseDate);
	Msg = Msg + TimeSeparator + DoField(ErrorDate.GetSecond(), S, wxT("??"),
	  ABaseDate);
	Msg = Msg + DecimalSeparator + DoField(ErrorDate.GetMillisecond(), MS,
	  wxT("???"), ABaseDate);
	throw EConvertError::CreateFmt(wxString::Format(SErrInvalidTimeStamp,
	  Msg.c_str()));
}

void InvalidDateTimeError(const unsigned short & AYear,
  const unsigned short & AMonth, const unsigned short & ADay,
  const unsigned short & AHour, const unsigned short & AMinute,
  const unsigned short & ASecond, const unsigned short & AMilliSecond,
  const wxDateTime & ABaseDate)
{
	...
	Msg = Msg + DecimalSeparator + DoField(AMilliSecond, MS, wxT("???"),
	  ABaseDate);
	throw EConvertError::CreateFmt(wxString::Format(SErrInvalidTimeStamp,
	  Msg.c_str()));
}
...
{
	InvalidDateTimeError(AYear, AMonth, ADay, AHour, AMinute, ASecond,
	  AMilliSecond, wxDefaultDateTime);
}

void InvalidDateWeekError(const unsigned short & AYear,
  const unsigned short & AWeekOfYear, const unsigned short & ADayOfWeek)
{
	throw EConvertError::CreateFmt(wxString::Format(SErrInvalidDateWeek, AYear,
	  AWeekOfYear, ADayOfWeek));
}

void InvalidDateDayError(const unsigned short & AYear,
  const unsigned short & ADayOfYear)
{
	throw EConvertError::CreateFmt(wxString::Format(SErrInvalidDayOfYear, AYear,
	  ADayOfYear));
}

void InvalidDateMonthWeekError(const unsigned short & AYear,
  const unsigned short & AMonth, const unsigned short & AWeekOfMonth,
  const unsigned short & ADayOfWeek)
{
	throw EConvertError::CreateFmt(wxString::Format(SErrInvalidDateMonthWeek,
	  AYear, AMonth, AWeekOfMonth, ADayOfWeek));
}

void InvalidDayOfWeekInMonthError(const unsigned short & AYear,
  const unsigned short & AMonth, const unsigned short & ANthDayOfWeek,
  const unsigned short & ADayOfWeek)
{
	throw EConvertError::CreateFmt(wxString::Format(SErrInvalidDayOfWeekInMonth,
...
wxDateTime JulianDateToDateTime(const double & AValue)
{
	...
	if (TryJulianDateToDateTime(AValue, Result) == false)
	{
		throw EConvertError::CreateFmt
		  (wxString::Format(wxT("Invalid Julian Date %f"), AValue));
	}
	return Result;
}
...
bool TryModifiedJulianDateToDateTime(const double & AValue,
  wxDateTime & ADateTime)
{
	wxUnusedVar(AValue);
	wxUnusedVar(ADateTime);
	// Fixme: Important
	THROW_NOT_IMPLEMENTED_ERROR;
}
